...
fn test_get_latency_badge_url() {
    ...
    assert_eq!(
        url,
        "https://status.example.org/api/v1/endpoints/core_service-1/response-times/24h/badge.svg"
    );
}
...
fn test_get_latency_chart_url() {
    ...
    assert_eq!(
        url,
        "https://status.example.org/api/v1/endpoints/core_service-1/response-times/24h/chart.svg"
    );
}
